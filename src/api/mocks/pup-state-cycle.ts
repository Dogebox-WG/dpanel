...
/** A [installation, status] pair applied per step of a mock lifecycle. */
export type MockCycleStep = [string, string];
export type MockCycle = MockCycleStep[];

export interface MockStatusUpdate {
  id: string;
  error: string;
  type: string;
  update: Record<string, { installation: string; status: string }>;
}

const msg: MockStatusUpdate = {
  "id": "",
  "error": "",
  "type": "status",
  "update": {}
...
function asStatusUpdate(pupId: string, newState: MockCycleStep): MockStatusUpdate {
  const out = { ...msg }
  out.update[pupId] = {
...
export async function performMockCycle(
  cycle: MockCycle,
  fn: (statusUpdate: MockStatusUpdate) => void | Promise<void>,
): Promise<void> {
