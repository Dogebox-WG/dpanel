...
interface MockVersionEntry {
  version: string;
  releaseNotes?: string;
  releaseDate?: string;
}

export interface MockPupUpdatesConfig {
  latestVersion: string;
  currentVersion: string;
  updateAvailable: boolean;
  availableVersions: MockVersionEntry[];
}

interface MockVersionDetails {
  version: string;
  releaseNotes: string;
  releaseDate: Date;
  releaseUrl: string;
  breakingChanges: unknown[];
  interfaceChanges: unknown[];
}

interface MockPupUpdateInfo {
  pupId: string;
  currentVersion: string;
  latestVersion: string;
  availableVersions: MockVersionDetails[];
  updateAvailable: boolean;
  lastChecked: Date;
}

// Default mock configuration
const defaultMockConfig: MockPupUpdatesConfig = {
  latestVersion: '1.2.0',
...
export function getMockConfig(): MockPupUpdatesConfig {
  try {
...
export function saveMockConfig(config: MockPupUpdatesConfig) {
  try {
...
function buildAvailableVersions(config: MockPupUpdatesConfig): MockVersionDetails[] {
  // Start with the explicit available versions
  let versions: MockVersionDetails[] = config.availableVersions.map(v => ({
    version: v.version,
...
function getInstalledPupIds(): string[] {
  try {
    ...
    if (pups && pups.length > 0) {
      ...
      const ids = pups
        .filter(pup => {
          const hasState = Boolean(pup.state && pup.state.id);
          return hasState;
        })
...
export const mockPupUpdatesApi = {
  getAllPupUpdates: async () => {
    ...
    const result: Record<string, MockPupUpdateInfo> = {};
  ...
  getPupUpdate: async (pupId: string) => {
  ...
  checkPupUpdates: async (pupId: string) => {
    await new Promise(resolve => setTimeout(resolve, 1000));
    const result = { jobId: 'job-' + Math.random().toString(36).substr(2, 9) };
    return result;
  },

  upgradePup: async (pupId: string, targetVersion?: string) => {
    await new Promise(resolve => setTimeout(resolve, 500));
    const result = { jobId: 'job-' + Math.random().toString(36).substr(2, 9) };
    return result;
  },

  rollbackPup: async (pupId: string) => {
    await new Promise(resolve => setTimeout(resolve, 500));
    const result = { jobId: 'job-' + Math.random().toString(36).substr(2, 9) };
    return result;
  },

  getPreviousVersion: async (pupId: string) => {
    await new Promise(resolve => setTimeout(resolve, 300));
    const result = {
      version: '0.9.0',
      manifest: {
  ...
  skipPupUpdate: async (pupId: string) => {
    await new Promise(resolve => setTimeout(resolve, 300));
    const result = { status: 'success' };
    return result;
  },

  clearSkippedUpdate: async (pupId: string) => {
    await new Promise(resolve => setTimeout(resolve, 300));
    const result = { status: 'success' };
    return result;
  }
