...
export const storeListingMock: MockDescriptor = {
  name: '/sources/store',
  method: 'get',
  group: 'sources',
...
interface MockStoreListingSource {
  lastUpdated: string;
  pups: Record<string, {
    latestVersion: string;
    versions: Record<string, ReturnType<typeof generateVersionData>>;
  }>;
}

function _generateStoreListingResponse(sources: Record<string, string[]>): Record<string, MockStoreListingSource> {
  const response: Record<string, MockStoreListingSource> = {};
...
function generateVersionData(version: string, pupName: string) {
  return {
