...
class ChangePassView extends LitElement {
  ...
  declare resetMethod: string | undefined;
  declare fieldDefaults: { resetMethod?: number };
  declare showSuccessAlert: boolean;
  declare refreshAfterChange: boolean;
  declare retainHash: boolean;
  declare noSubmit: boolean;
  declare onBack: (() => void) | null;
  declare _server_fault: boolean;
  declare _invalid_creds: boolean;

  onSuccess: (() => void) | null;
  _changePassFields?: { sections: { name: string; submitLabel: string; fields: FormFieldDef[] }[] };
  ...
  connectedCallback() {
    ...
    const changePassFields: NonNullable<ChangePassView["_changePassFields"]> = {
      sections: [
        {
  ...
  _attemptChangePass = async (data: ChangePassFormData, form: unknown, dynamicFormInstance: DynamicFormInstance) => {
