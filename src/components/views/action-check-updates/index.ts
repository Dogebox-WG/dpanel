...
import type { PackageUpdate, UpdatablePackage } from "/api/system/updates.js";
import type { ActionProgress, JobRecord } from "/types/jobs.js";

/** UpdatablePackage with latestUpdate resolved to its full update entry. */
type ResolvedUpdatablePackage = Omit<UpdatablePackage, "latestUpdate"> & {
  latestUpdate: PackageUpdate | undefined;
};
...
export class CheckUpdatesView extends LitElement {
  ...
  declare _update_outcome: string | null;
  declare _page: string;
  declare _logs: UpdateLogEntry[];
  declare _systemJobId: string;

  context: StoreSubscriber;
  pkgController: typeof pkgController;
  _updatePollId: ReturnType<typeof setInterval> | null;
  _updatablePackages: ResolvedUpdatablePackage[] = [];
  ...
  extractSystemJobId(logs: UpdateLogEntry[]) {
    if (!Array.isArray(logs)) return "";
    return logs.findLast((log) => log?.actionID)?.actionID ?? ""
  }

  isSystemJobErrorLog(log: UpdateLogEntry) {
  ...
  checkProgressFailure(logs: UpdateLogEntry[]) {
  ...
  async pollForUpdateChange() {
    ...
    this._updatePollId = setInterval(async () => {
      try {
        ...
        const dbxVersion = store.getContext('app')?.dbxVersion;
