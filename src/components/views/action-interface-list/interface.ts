...
export class InterfaceList extends LitElement {
  declare interfaces: PupInterface[];
  declare expandedInterface: string | null;
  ...
  handleExpand(interfaceName: string) {
  ...
  render() {
    ...
    const renderSeverityOptions = (severityNumber: number | undefined) => {
        ...
        <sl-card class="permission-group">
          ...
          ${group.routes && group.routes.length ? html`
            ...
            <div class="routes">
              ...
              <div class="value">${group.routes.map((r: string) => html`${r}`)}</div>
            </div>`: nothing
          }
          <sl-divider></sl-divider>
          <div class="severity">
