...
class PupInstallCard extends LitElement {
  ...
  set installationState(newState: InstallationStateProp | undefined) {
    this._installationState = newState;
    this.requestUpdate();
  }

  renderSourceIcon(sourceType: string | undefined) {
    let icon;
    switch (sourceType) {
      case 'git':
        icon = 'git';
